...
function readData() {
    ...
    files.forEach(f => {
        ...
        let data = Object.keys(json).map(k => {
            ...
            if (data.trend.community&&data.trend.community.length>0) {
                community = parseInt(Object.entries(data.trend.community.pop())[0][1], 10);
            }
            if (data.trend.area&&data.trend.area.length>0) {
    ...
    var txt = JSON.stringify(list); // "module.exports=" + JSON.stringify( points );
    fs.writeFile(path.resolve(__dirname, `../../web/libs/data/fccs.json`), txt, function (err) {
        if (err)
            console.log("\tfail " + err);
        else {
            console.log("\t写入文件ok");
